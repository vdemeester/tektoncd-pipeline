...
func EnsureConfigurationConfigMapsExist(d *Data) {
	var defaultsExists, featureFlagsExists, metricsExists, spireconfigExists, eventsExists, tracingExists, backoffExists, artifactStorageExists bool
	for _, cm := range d.ConfigMaps {
		...
		if cm.Name == config.GetArtifactStorageConfigName() {
			artifactStorageExists = true
		}
	}
	if !defaultsExists {
