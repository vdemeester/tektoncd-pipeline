...
links=$(find "$(dirname "$0")/../config/300-resources" -type l)
for link in $links; do
  cp "$link" "$link.bkp"
done

SCHEMAPATCH_CONFIG_FILE="$(dirname $0)/schemapatch-config.yaml" controller-gen \
  schemapatch:manifests=config/300-resources/,generateEmbeddedObjectMeta=true \
  output:dir=config/300-resources/ \
  paths=./pkg/apis/...
